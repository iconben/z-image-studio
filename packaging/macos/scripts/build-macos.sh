#!/bin/bash
# Cross-platform build script for Z-Image Studio macOS .app bundle and .dmg
...
if [ ! -f "$SPEC_FILE" ]; then
    ...
    echo "Listing packaging/macos/pyinstaller/:"
    ls -la packaging/macos/pyinstaller/ 2>/dev/null || echo "Directory not found"
    exit 1
fi
...
APP_PATH="dist/Z-Image Studio.app"
...
DMG_NAME="Z-Image-Studio-${VERSION}-macOS.dmg"
echo "Creating .dmg file: $DMG_NAME..."

APP_ICON="$RESOURCES_PATH/logo.icns"
if [ -f "src/zimage/static/logo-180.png" ]; then
    echo "Converting logo to icns format..."
    if command -v sips &> /dev/null; then
        sips -s format png "src/zimage/static/logo-180.png" --out "/tmp/logo-temp.png" > /dev/null 2>&1 || true
    fi
fi
...
create-dmg \
    ...
    --volicon "$RESOURCES_PATH/logo.icns" 2>/dev/null || true \
    --window-pos 200 120 \
    --window-size 800 600 \
    --icon-size 100 \
    ...
    "dist/$DMG_NAME" \
    "dist/Z-Image Studio.app" 2>/dev/null || \
create-dmg \
    --volname "Z-Image Studio" \
    --window-pos 200 120 \
    --window-size 800 600 \
